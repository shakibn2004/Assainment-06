...
const ToolsCards = ({
  ...
  const buyHandle = (item) => {
    if(!selectedProduct.find(selectedItem => selectedItem.name === item.name)) {
      ...
      toast.error("This product already exist in your card")
    }
...
setTotalPrice(0)
  ...
  const handleDelete = (item) => {
    const fiterProducts = selectedProduct.filter(filterItem => filterItem.name !== item.name);
      ...
      <div className="tool-top grid justify-items-center text-center">
        ...
        <div className="tool-btn grid grid-cols-2 gap-4 border border-[#F6F6F6] shadow-[0_0_2px_black] rounded-full p-1">
          <button
            ...
            className={`px-4 py-3 ${renderCard?'lenear-bg text-white':'bg-none'} rounded-full  font-bold`}
          >
            Products
          </button>
          <button onClick={() => setRenderCard(false)} className={`font-bold ${!renderCard?'lenear-bg text-white':'bg-none'} rounded-full`}>
      ...
      <div className="tool-bottom grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        ? (
          <div className="p-10 col-span-3 space-y-4 shadow-[0_0_2px_black] rounded-2xl">
            ...
            {selectedProduct.map((item) => {
              return (
                <div className="flex flex-col md:flex-row gap-8 md:gap-0 justify-between items-center p-6 bg-[#F9FAFC] rounded-2xl">
                  ...
                  <div className="right">
                    <h1 className="text-red-600 cursor-pointer" onClick={()=> handleDelete(item) }>DELETE</h1>
                  </div>
                </div>
              );
            })}
            ...
            <button onClick={()=>handleCheckOut()} className="lenear-bg text-white py-3 rounded-full w-full mt-6">
              Proceed to Checkout
            </button>
          </div>
        )
      : (
        <h1 className="text-3xl font-bold mx-auto col-span-3 mt-10">Your Cart Is Empty! Buy Something</h1>
      )}
      </div>
    </section>
  );
